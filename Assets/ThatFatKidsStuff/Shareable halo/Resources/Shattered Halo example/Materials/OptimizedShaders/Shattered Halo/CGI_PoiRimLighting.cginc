...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = float(0.05);
    float rimBrighten = float(0);
    float rimWidth = lerp( - .05, 1, float(0.427));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(1,0.7794898,0.3456162,1);
    #else
        rimColor = float4(1,0.7794898,0.3456162,1);
    #endif
    ...
    rim = 1 - smoothstep(min(float(0.09), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(1,0.7794898,0.3456162,1).a * rimColor.a * rimMask;
