Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/6bf8420672ce7314a969ca89247fd40c"
{
	Properties
	{
 ...
 #define PROP_GLITTERTEXTURE 
			...
			{
				float2 st = frac(poiMesh.uv[float(4)] + float4(0,0,0,0).xy * _Time.x) * float(100);
				...
				switch(float(1))
				{
					...
					break;
					case 1: //sqaure
					float jaggyFix = pow(poiCam.distanceToVert, 2) * float(0.1);
					
					if (float(0) == 1 || float(5) != 0)
					{
						...
						float theta = radians((randomBoy + _Time.x * float(5)) * 360);
						float cs = cos(theta);
						float sn = sin(theta);
				...
				switch(float(0))
				{
					...
					if (float(2) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(90), float(2));
					}
					else
					{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(149)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(2)) * float(20) - (float(20) - 1);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(149)), 0);
				...
				if (float(0) == 1 || float(5) != 0 && !float(1))
				{
				...
				#if defined(PROP_GLITTERMASK) || !defined(OPTIMIZER_ENABLED)
				float glitterMask = POI2D_SAMPLER_PAN(_GlitterMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.5,0,-0.1)), float4(0,0,0,0));
				#else
				float glitterMask = 1;
				#endif
